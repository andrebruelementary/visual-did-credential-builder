...
//import com.elementarysoftware.bip0039.BIP0039SeedPhrases;
...
import com.elementarysoftware.prism.jobs.PublishDIDToBlockchain;
...
public class MainWindow {
	...
	protected void openDIDProperyPage() {
		...
		/*while (!didDialog.getShell().isDisposed()) {
			if (!didDialog.getShell().getDisplay().readAndDispatch()) {
	...
	protected void createContents() {
		...
		btnPublishDid.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				PublishDIDJob job = new PublishDIDJob(loadedDID);
		...
		btnPublishDid.setEnabled(false);
		btnPublishDid.setBounds(281, 136, 159, 27);
		btnPublishDid.setText("Publish DID");*/
		
		/*
		btnUpdateDid = new Button(shlVisualDid, SWT.NONE);
		btnUpdateDid.setEnabled(false);
		btnUpdateDid.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				
				UpdateDIDJob job = new UpdateDIDJob(loadedDID);
				Thread t = new Thread(job);
				t.start();
		...
		/*
		ComboViewer comboViewer = new ComboViewer(shlVisualDid, SWT.NONE);
		Combo combo_1 = comboViewer.getCombo();
		combo_1.setBounds(217, 198, 181, 22);
		
		//comboViewer.setContentProvider(new com.elementarysoftware.bip0039.BIP0039SeedPhrases());
