...
public class AddKeyDialog extends Dialog {
	...
	private void suggestKeyName() {
		...
		if(comboKeyType.getSelectionIndex() == -1) return;
		
		String selectedKeyType = comboKeyType.getItem(comboKeyType.getSelectionIndex());
		/*if(selectedKeyType.equals(DIDKeyInfo.KEY_TYPE_AUTHENTICATION)) {
			keyType = PrismKeyType.INSTANCE.getAUTHENTICATION_KEY();
			defaultKeyName = PrismDid.get
		}
		else */if(selectedKeyType.equals(DIDKeyInfo.KEY_TYPE_ISSUING)) {
		...
		if(keyType == -1 || defaultKeyName.equals("")) { System.out.println("keyType: " + keyType +", defaultKeyName: "+ defaultKeyName); return; }
		...
		//TODO: IF POSSIBLE, REMOVE ALREADY USED INDEXES FROM THE LIST. CANNOT SEE ANY WAY TO GET KEY INDEX CURRENTLY 
		
		/*
		//int availableIndexes
		
		// check type of key selected for creation and translate to int type
		int keyType = -1;
		...
		for(int i = 0; i < keyinfo.length; i++) {
			...
			if(tmpKey.getKeyTypeEnum() == keyType) {
				//tmpKey.getKeyId().indexOf(selectedKeyType)
				tmpKey.component3().
			}
		}*/
	}
	...
	protected void okPressed() {
		...
		for(int i = 0; i < keyinfo.length; i++) {
			...
			if(tmpKey.getKeyId().toLowerCase().equals(textKeyName.getText().toLowerCase().trim())) {
				MessageDialog.openWarning(getParentShell(), "Duplicate key name", "Key with name "+ textKeyName.getText() +" already exist. Please select unique name");
				return;
			}
		}
